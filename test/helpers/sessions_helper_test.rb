...
class SessionsHelperTest < ActionView::TestCase
    ...
    test"current_user returns right user when sessions is nil" do
      ...
      # current_userがnilと判定されてテストが通らない。
      # 解決法：sissions_helperに２つあるcurrent_userメソッド(一時sessionsと永続的cokies)の順番を入れ替えた。
      assert is_logged_in?
    ...
    test "current_user returns nil when remember digest is wrong" do
      ...
      # @userの記憶ダイジェストが、ハッシュ化した記憶トークンを暗号化した値と同じなら、記憶ダイジェストを更新する
